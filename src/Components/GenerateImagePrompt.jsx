...
export default function GenerateImagePrompt() {
    const { post, setPost, loading, postLoading } = useContext(ContextApi)
    return (
        <div className='md:w-[50%]'>
            ...
            <div className='flex flex-col mt-5 gap-2'>
                ...
                <input type="text" name="name" id="name" value={post.author} onChange={(e) => { setPost({ ...post, author: e.target.value }) }} placeHolder="Enter your name" className='bg-transparent border-1 border-[#c4c5c577] md:w-[85%] rounded-lg text-white outline-none py-2 px-5' />
            </div>
            ...
            <div className='md:w-[83%] mt-5 flex justify-between'>
                <button disabled={post.prompt == '' || post.author == ''} onClick={() => console.log('dsdsd')} className='w-[48%] cursor-pointer flex gap-1 justify-center items-center rounded-lg py-2 text-white font-[500] md:text-lg bg-gradient-to-r from-[#215AF8] from-100%'>
                    {
                        loading ? (
                            <>
                                <Loading />
                ...
                <button disabled={post.prompt == '' || post.author == ''} className='w-[48%] cursor-pointer flex justify-center gap-1 items-center rounded-lg py-2 text-white font-[500] md:text-lg bg-gradient-to-r from-purple-600 to-purple-700'>
                    {
                        postLoading ? (
                            <>
                                <PostLoading />
